...
### 3.1 動画アップロード・解析 (Core)
*   **F-01 動画アップロード:** ユーザーがMP4ファイルをドラッグ＆ドロップでアップロードできる。
*   **F-02 フレーム抽出:** バックエンド側で動画を一定間隔（例: 1fps）で画像化する。
*   **F-03 高速シーン解析 (SambaNova):**
    *   抽出した画像をSambaNova API (Llama 3.2 Vision) に投げ、以下のJSONを取得する。
    *   `{ "timestamp": "00:12:05", "action": "Clipping", "instruments": ["Clip applier"], "description": "胆嚢管へのクリッピング" }`
*   **F-04 ガイドライン照合 (vLLM + RAG):**
    *   解析されたアクションに対し、医学ガイドライン（事前にVector DB化）を検索し、手技のポイントを付与する。

### 3.2 ダッシュボード・検索 (UI/UX)
*   **F-05 インタラクティブ・タイムライン:**
    *   動画プレイヤーの下に、手技ごとの「色分けされたバー」を表示する（例：赤=切開、青=縫合）。
    *   バーをクリックすると、そのシーンへ即座にシーク（移動）する。
*   **F-06 アクション検索:**
    *   検索窓に「結紮（けっさつ）」と入力すると、タイムライン上の該当箇所のみがハイライトされる。
    *   検索結果リストが表示され、クリックで再生開始。

### 3.3 AI解説・チャット
*   **F-07 AIコメンタリー:**
    *   再生中のシーンに合わせて、画面横に「AIによる解説（このシーンの注意点）」と「ガイドラインの抜粋」をリアルタイム表示する。
*   **F-08 質疑応答:**
    *   「この時の剥離操作のコツは？」とチャットで聞くと、Llamaがガイドラインを参照して回答する。

## 4. 非機能要件 (Non-Functional Requirements)

*   **NF-01 パフォーマンス:** アップロード後、動画時間の50%以下の時間で解析を完了させる（SambaNovaの速度を活かす）。
*   **NF-02 UIデザイン:** 医療現場で使われることを意識した「ダークモード」基調のProfessionalな配色（黒・グレー・エメラルドグリーン）。
*   **NF-03 レスポンス:** 検索結果の表示は0.5秒以内。

## 5. データ要件

*   **動画データ:** 腹腔鏡下胆嚢摘出術（Laparoscopic Cholecystectomy）の動画（MP4, 5〜10分にトリミング済み）。
*   **テキストデータ:** 同手術の標準ガイドラインテキスト（PDFから抽出）。
*   **プロンプト:**
...
*   **Must (必須 - Day 1完了目標):**
    *   動画の画像切り出し処理。
    *   SambaNovaによるシーン解析とJSON化。
    *   FastAPIによる解析結果のAPI提供。
    *   Next.jsによる動画再生とタイムライン表示（検索機能含む）。
*   **Should (推奨 - Day 2目標):**
